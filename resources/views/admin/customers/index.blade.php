...
<a href="{{ route('admin.customers.create') }}" class="btn btn-primary waves-effect waves-float waves-light btn-block">
...
<div class="row" id="table-hover-row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form class="kb-search-input" action="{{ route('admin.customers.index') }}" method="get">
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <a class="dropdown-item" href="{{ route('admin.customers.show', ['id' => $user->id]) }}">
                                            <i data-feather="eye" class="mr-50"></i>
                                            <span>Show</span>
                                        </a>
                                        @endif
                                        @if (Auth::user()->hasPermission('update customers'))
                                        <a class="dropdown-item" href="{{ route('admin.customers.edit', ['id' => $user->id]) }}">
                                            <i data-feather="edit-2" class="mr-50"></i>
                                            <span>Edit</span>
                                        </a>
                                        @endif
                                        @if (Auth::user()->hasPermission('delete customers'))
                                        <form action="{{ route('admin.customers.destroy', ['id' => $user->id]) }}" method="post">
                                            @method('delete')
                                            @csrf
